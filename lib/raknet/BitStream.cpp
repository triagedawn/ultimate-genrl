...
void BitStream::WriteOrthMatrix( float m00, float m01, float m02, float m10, float m11, float m12, float m20, float m21,
								 float m22 ) {
	double qw;
	double qx;
	double qy;
...
bool BitStream::Read( bool &output ) {
	...
	if ( ReadBits( (unsigned char *)&ID, sizeof( unsigned char ) * 8 ) == false ) return false;
...
#	endif
	...
	// assert(readOffset+1 <=numberOfBitsUsed); // If this assert is hit the stream wasn't long enough to read
	// from
...
bool BitStream::Read( unsigned int &output ) {
	...
	if ( ReadBits( uint32r, sizeof( output ) * 8 ) != true ) return false;
	output = ( ( (unsigned int)uint32r[B32_3] ) << 24 ) | ( ( (unsigned int)uint32r[B32_2] ) << 16 ) |
			 ( ( (unsigned int)uint32r[B32_1] ) << 8 ) | ( (unsigned int)uint32r[B32_0] );
	return true;
#endif
}
...
bool BitStream::Read( int &output ) {
	...
	if ( ReadBits( int32r, sizeof( output ) * 8 ) != true ) return false;
	output = ( ( (unsigned int)int32r[B32_3] ) << 24 ) | ( ( (unsigned int)int32r[B32_2] ) << 16 ) |
			 ( ( (unsigned int)int32r[B32_1] ) << 8 ) | ( (unsigned int)int32r[B32_0] );
	return true;
#endif
}
...
bool BitStream::ReadCompressed( unsigned short &output ) {
	...
	output = ( ( (unsigned short)uint16rc[B16_1] ) << 8 ) | ( (unsigned short)uint16rc[B16_0] );
	return true;
#endif
}
...
bool BitStream::ReadCompressed( unsigned int &output ) {
	...
	if ( ReadCompressed( uint32rc, sizeof( output ) * 8, true ) != true ) return false;
	output = ( ( (unsigned int)uint32rc[B32_3] ) << 24 ) | ( ( (unsigned int)uint32rc[B32_2] ) << 16 ) |
			 ( ( (unsigned int)uint32rc[B32_1] ) << 8 ) | ( (unsigned int)uint32rc[B32_0] );
	return true;
#endif
}
...
bool BitStream::ReadCompressed( int &output ) {
	...
	if ( ReadCompressed( int32rc, sizeof( output ) * 8, false ) != true ) return false;
	output = ( ( (unsigned int)int32rc[B32_3] ) << 24 ) | ( ( (unsigned int)int32rc[B32_2] ) << 16 ) |
			 ( ( (unsigned int)int32rc[B32_1] ) << 8 ) | ( (unsigned int)int32rc[B32_0] );
	return true;
#endif
}
...
bool BitStream::ReadOrthMatrix( float &m00, float &m01, float &m02, float &m10, float &m11, float &m12, float &m20,
								float &m21, float &m22 ) {
	float qw, qx, qy, qz;
...
#pragma warning( disable : 4800 )
	auto res = (bool)( data[readOffset >> 3] & ( 0x80 >> ( readOffset % 8 ) ) );
	readOffset++;
	return res;
...
// Align the next write and/or read to a byte boundary.  This can be used to 'waste' bits to byte align for
// efficiency reasons
...
void BitStream::WriteBits( const unsigned char *input, int numberOfBitsToWrite, const bool rightAlignedBits ) {
	...
	// do
	{
		...
		if ( numberOfBitsToWrite < 8 &&
			 rightAlignedBits ) // rightAlignedBits means in the case of a partial byte, the bits are aligned
								// from the right (bit 0) rather than the left (as in the normal internal
								// representation)
...
void BitStream::WriteCompressed( const unsigned char *input, const int size, const bool unsignedData ) {
	...
	// From high byte to low byte, if high byte is a byteMatch then write a 1 bit. Otherwise write a 0 bit and
	...
	// If the upper half of the last byte is a 0 (positive) or 16 (negative) then write a 1 and the remaining
	// 4 bits.  Otherwise write a 0 and the 8 bites.
...
bool BitStream::ReadBits( unsigned char *output, int numberOfBitsToRead, const bool alignBitsToRight ) {
	...
	while ( numberOfBitsToRead > 0 ) {
		*( output + offset ) |= *( data + ( readOffset >> 3 ) ) << ( readOffsetMod8 ); // First half
		...
		if ( readOffsetMod8 > 0 &&
			 numberOfBitsToRead > 8 - ( readOffsetMod8 ) ) // If we have a second half, we didn't read enough
...
bool BitStream::ReadCompressed( unsigned char *output, const int size, const bool unsignedData ) {
	...
	// All but the first bytes are byteMatch.  If the upper half of the last byte is a 0 (positive) or 16
	// (negative) then what we read will be a 1 and the remaining 4 bits. Otherwise we read a 0 and the 8
	// bytes
	// assert(readOffset+1 <=numberOfBitsUsed); // If this assert is hit the stream wasn't long enough to read
	// from
	if ( readOffset + 1 > numberOfBitsUsed ) return false;
...
// If we used the constructor version with copy data off, this makes sure it is set to on and the data pointed
// to is copied.
