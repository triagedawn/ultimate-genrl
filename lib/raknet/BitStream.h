...
public:
	...
	*/
	void WriteOrthMatrix( float m00, float m01, float m02, float m10, float m11, float m12, float m20, float m21,
						  float m22 );
	/**
	 * Write the native types with simple compression.
	 * Best used with  negatives and positives close to 0
	...
	* for 6 bytes instead of 36
	*/
	bool ReadOrthMatrix( float &m00, float &m01, float &m02, float &m10, float &m11, float &m12, float &m20, float &m21,
						 float &m22 );
	/**
	 * Read the types you wrote with WriteCompressed
	 * @param output The read value
	...
	*/
	...
	void SetReadOffset( int newReadOffset ) { readOffset = newReadOffset; }
	/**
